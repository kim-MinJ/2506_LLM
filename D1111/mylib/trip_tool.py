...
from langchain_core.tools import tool # 안씀
import logging
import json # 안씀
...
# 세션 기반 에이전트 실행 함수 (이전 질문 기억 + 도구 사용)
def run_agent(user_input: str, session_id: str):
    """
    세션 기반으로 이전 대화 기록을 유지하며 에이전트를 실행
    """
    print(f"\n{'='*70}")
    print(f"👤 사용자({session_id}): {user_input}")
    print(f"{'='*70}")
    ...
    try:
        ...
        # 6️⃣ 출력
        print(f"\n{'='*70}")
        print(f"🤖 어시스턴트({session_id}): {final_answer}")
        print(f"{'='*70}\n")
...
if __name__ == "__main__":
    ...
    # 1. 시간 조회
    final_answer=run_agent("지금 현재 시간이 몇 시야?", session_id)
    ...
    # 3. 검색
    final_answer=run_agent("AI Agent 가 무엇인지 한 줄로 알려줘", session_id)
    print(f'💬 : {final_answer}')
    
    # 4. 복합 질문 (여러 도구 조합)
    # run_agent("지금 시간이 몇 시고, 부산의 날씨는 어떻게 되는지 알려주세요", session_id)
    
    # 5. 정보 검색과 날씨
    final_answer=run_agent("충북 괴산의 날씨를 알려주고, 2025년 11월 괴산의 축제 일정을 알려줘", session_id)
    print(f'💬 : {final_answer}')