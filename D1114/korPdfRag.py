...
# ✅ KoreanEmbedding의 속성 : embeddings
...
# ✅ PdfRAGSystem의 속성 : index_name, pc(파인콘 객체), embeddings, llm, vectorstore
class PdfRAGSystem:
    ...
    def load_and_split_pdf(
        ...
        Args:
            pdf_path: PDF 파일 경로
            chunk_size: 청크 크기 (한글용으로 500~800으로 조정)
        ...
        # 이미지 텍스트 제거: 빈 페이지나 텍스트 없는 페이지 필터링
        ...
        rag_chain = (
            {
                ...
                # 체인안의 체인 : 사용자 쿼리를 검색할 수 있게 벡터화하여 유사도 검색
...
def main(run:bool,pdf_path:str,index_name:str,questions:List[str]):
    ...
    # 새로운 문서 임베딩하기
    if run:  # 참이면 index_name에 대해 업서트(create_vectorstore 메소드 실행)
